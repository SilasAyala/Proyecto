...
const signup = async (req, res, next) => {
  ...
  //creamos hash de contrasena
  let hashedPassword;
  try{
    hashedPassword = await bcrypt.hash(password, 12);
  } catch(err) {
    const error = 
      HttpError('No se puedo crear el usuario, intenta de nuevo.', 500);
    return next(error);
  }
  ...
  try{
    ...
    console.log(err);
    const error = new HttpError(
      'Fallo el Signup, por favor intenta de nuevo.',
      500
    )
  }
...
const login = async (req, res, next) => {
  ...
  try{
    ...
    console.log(err);
    const error = 
      new HttpError('No se puedieron validar los datos, intente de nuevo,', 500);
    return next(error);
  }

  if(!isValidPassword){
    const error = 
      new HttpError('Credenciales invalidas, revisa e intenta de nuevo.', 401);
    return next(error);
  }

  //logica de manejo de tokens
  let token;
  try{
    token = jwt.sign(
    ...
    const error = 
      new HttpError('Fallo el Login, intenta de nuevo.', 500);
    return next(error);
  }
